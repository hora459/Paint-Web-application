...
export class AppComponent {
  ...
  resize(){
    ...
    this.shape1.on('transformend', ()=> {
      ...
      this.shapefromback=this.builder.MakeObject(this.shape1.className,this.shape1)
    ...
    if (this.fillflag == true) {
      if (shape != undefined) {
        ...
        if (shape.className == 'Rect' && shape.width == shape.height) {
          ...
          this.shape2 = this.builder.MakeObject("Square", shape);
        } 
        else {
          this.shape2 = this.factory.getshape(shape.className);
          this.shape2 = this.builder.MakeObject(shape.className, shape);
        }
        this.fillflag = false;
        this.tr.nodes([]);

...
import { Objectmaker } from './paint/objectmaker';
...
export class AppComponent {
  ...
  resize(){
    ...
    this.stage.on('click', (e: any) => {
      ...
      if (shape == undefined) {
        this.tr.nodes([]);
        this.currentpositon = e.currenttarget;
    ...
    this.shape1.on('transformend', ()=> {
      this.shapefromback=this.Object_maker.MakeObject(this.shape1.className,this.shape1)
    ...
    if (shapename == 'rectangle') {
      ...
      if (this.squareflag == true) {
        this.shape1 = new konva.Rect({
          ...
          scaleY: this.shapefromback.scaleY,
          fill: this.shapefromback.fill,
          rotation: this.shapefromback.rotation,
          strokeScaleEnabled: false,
          draggable: false
        });
        this.layer.add(this.shape1);
    ...
    if (this.fillflag == true) {
      if (shape != undefined) {
        ...
        if (shape.className == 'Rect' && shape.width == shape.height) {
          ...
          this.shape2 = this.Object_maker.MakeObject("Square", shape);
        } 
        else {
          this.shape2 = this.factory.getshape(shape.className);
          this.shape2 = this.Object_maker.MakeObject(shape.className, shape);
        }
        this.fillflag = false;
        this.tr.nodes([]);
  ...
  clone(){
    ...
    this.stage.on('click', (e: any) => {
      ...
      if (this.cloneflag == true) {
        if (shape != undefined) { 
          this.drawit.clone(this.event_id,++this.id).subscribe(res=>{ this.drawback(res.type,res)});
            }
